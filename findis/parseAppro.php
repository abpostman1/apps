...
// Récupération de la QTY en commande fournisseur (réappro)
// PARSE_CAT.PHP et PARSE.PHP doivent être exécuté préalablement.
...
if (($handle = fopen("source/exportArticles.csv", "r")) !== FALSE) {
    while (($data = fgetcsv($handle, ";")) !== FALSE) {
	    $sku = htmlspecialchars(mysqli_real_escape_string($mysqli,$data[9]));
        $dispo = htmlspecialchars(mysqli_real_escape_string($mysqli,$data[9]));
		$reappro = htmlspecialchars(mysqli_real_escape_string($mysqli,$data[1]));
...
while (isset($xml->prods->prod[$count])) {
	...
	$lib = str_replace(' null ', ' ', htmlspecialchars(mysqli_real_escape_string($mysqli,$xml->prods->prod[$count]->lib))) ;
